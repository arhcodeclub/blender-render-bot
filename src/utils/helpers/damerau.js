...
function damerau(a, b) {
    if (a.length === 0)
        return { steps: b.length, relative: 1, similarity: 0 };
    if (b.length === 0)
        return { steps: a.length, relative: 1, similarity: 0 };
    const matrix = [];
    let i;
...
function quickSort(arr, left, right, query) {
    if (left >= right)
        return;
 ...
 */
export function damerAutocomplete(query, choices) {
    if (!choices)
        return [];
    const levChoices = [];
    ...
    const filtered = levChoices.filter((choice) => {
        const lev = damerau(choice, query);
        if (query.length > 2)
            return lev.relative <= 0.75;
        else if (query.length > 1)
            return lev.relative <= 0.8;
        else
            return lev.relative <= 1;
    });
...
//# sourceMappingURL=damerau.js.map